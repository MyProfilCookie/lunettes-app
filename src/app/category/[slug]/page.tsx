    ...
    <div className="min-h-screen overflow-x-hidden" style={{ background: 'var(--bg)', color: 'var(--fg)' }}>
      ...
      <section className="relative" style={{ borderBottom: '1px solid var(--border)' }}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
          <div className="flex items-center gap-2 text-[12px] sm:text-[13px] mb-4 sm:mb-6 text-zinc-500">
          ...
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl flex items-center justify-center shrink-0" style={{ background: 'var(--accent-light)' }}>
              <svg className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: 'var(--accent)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d={icon} />
              </svg>
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-white">{category.name}</h1>
              <p className="text-[12px] sm:text-sm mt-0.5 text-zinc-500">
        ...
        <div className="flex gap-8 overflow-hidden">
          ...
          <main className="flex-1 min-w-0">
                ...
                {allCategories.map((cat) => {
                  ...
                  return (
                    <Link
                      key={cat.slug}
                      href={`/category/${cat.slug}`}
                      className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-[12px] sm:text-[13px] font-medium whitespace-nowrap transition-all"
                      style={{
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              ...
              <select
                className="text-[12px] sm:text-[13px] font-medium rounded-lg sm:rounded-xl px-2.5 sm:px-3 py-1.5 sm:py-2 focus:outline-none"
            ...
            {category.glasses.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4 animate-stagger">
